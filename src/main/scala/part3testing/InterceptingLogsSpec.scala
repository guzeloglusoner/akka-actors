...
import part2actors.ActorLoggingDemo.ActorWithLogging
...
class InterceptingLogsSpec extends TestKit(
  ...
  "A checkout flow" should {
    "correctly log the dispatch of an order" in {
      // intercept only waits for 3 seconds
      // we can also debug warning error
...
object InterceptingLogsSpec {
  ...
  class CheckoutActor extends Actor {
    ...
    def pendingPayment(item: String): Receive = {
      ...
      case PaymentDenied => throw new RuntimeException("I cant handle this anymore")
    }
